...
import { CLI_BINARY_NAME, getCliBinaryFilename } from '../../constants/product.js'
...
export function getBinaryName(platform: string): string {
  return platform.startsWith('win32') ? `${CLI_BINARY_NAME}.exe` : CLI_BINARY_NAME
}
...
async function manualRemoveNpmPackage(
  ...
  try {
    ...
    if (getPlatform().startsWith('win32')) {
      ...
      const binCmd = join(globalPrefix, `${CLI_BINARY_NAME}.cmd`)
      const binPs1 = join(globalPrefix, `${CLI_BINARY_NAME}.ps1`)
      const binExe = join(globalPrefix, CLI_BINARY_NAME)
      ...
      const binSymlink = join(globalPrefix, 'bin', CLI_BINARY_NAME)

...
async function getInstallationPath(): Promise<string> {
  ...
  if (isInBundledMode()) {
    ...
    try {
      const path = await which(CLI_BINARY_NAME)
      if (path) {
        return path
      }
    } catch {
...
> {
  ...
  if (npmResult.code === 0 && npmResult.stdout) {
    ...
    const globalBinPath = isWindows
      ? join(npmPrefix, CLI_BINARY_NAME)
      : join(npmPrefix, 'bin', CLI_BINARY_NAME)
  ...
  const nativeBinPath = join(homedir(), '.local', 'bin', CLI_BINARY_NAME)
  try {
  ...
  if (!isEnvTruthy(process.env.DISABLE_INSTALLATION_CHECKS)) {
    if (type === 'npm-local' && config.installMethod !== 'local') {
      warnings.push({
        ...
        fix: `Consider using native installation: ${CLI_BINARY_NAME} install`,
      })
    }
    ...
    if (type === 'native' && config.installMethod !== 'native') {
      warnings.push({
        ...
        fix: `Run ${CLI_BINARY_NAME} install to update configuration`,
      })
    }
  }
  ...
  if (installationType === 'npm-global') {
    ...
    if (!hasUpdatePermissions && !getAutoUpdaterDisabledReason()) {
      warnings.push({
        ...
        fix: `Do one of: (1) Re-install node without sudo, or (2) Use \`${CLI_BINARY_NAME} install\` for native installation`,
      })
    }
  }

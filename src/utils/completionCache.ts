...
export async function setupShellCompletion(theme: ThemeName): Promise<string> {
  ...
  try {
    ...
    logError(e)
    return `${EOL}${color('warning', theme)(`Could not write ${shell.name} completion cache`)}${EOL}${chalk.dim(`Run manually: ${CLI_BINARY_NAME} completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  const claudeBin = process.argv[1] || CLI_BINARY_NAME
  ...
  if (result.code !== 0) {
    return `${EOL}${color('warning', theme)(`Could not generate ${shell.name} shell completions`)}${EOL}${chalk.dim(`Run manually: ${CLI_BINARY_NAME} completion ${shell.shellFlag} > ${shell.cacheFile}`)}${EOL}`
  }
  ...
  try {
    ...
    if (
      existing.includes(`${CLI_BINARY_NAME} completion`) ||

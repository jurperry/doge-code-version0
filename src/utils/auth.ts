...
import { CLI_BINARY_NAME } from 'src/constants/product.js'
import {
...
export function getAnthropicApiKeyWithSource(
  ...
  const persistedCustomApiKey =
    customApiStorage.apiKey || getGlobalConfig().customApiEndpoint?.apiKey
  const effectiveApiKeyEnv = apiKeyEnv || persistedCustomApiKey
  const compatProviderEnv = process.env.CLAUDE_CODE_COMPATIBLE_API_PROVIDER
  const customProvider =
    customApiStorage.provider ??
    (compatProviderEnv === 'openai' || compatProviderEnv === 'responses'
      ? compatProviderEnv
      : 'anthropic')
  ...
  // If using a custom API provider (OpenAI-compatible or Anthropic-compatible),
  // return the API key without requiring approval
  if (effectiveApiKeyEnv && (customProvider === 'openai' || !isFirstPartyAnthropicBaseUrl())) {
    return {
      key: effectiveApiKeyEnv,
      source: 'DOGE_API_KEY',
    }
...
export async function validateForceLoginOrg(): Promise<OrgValidationResult> {
  ...
  if (!profile) {
    ...
    return {
      ...
      message:
        ...
        `verification (tokens from '${CLI_BINARY_NAME} setup-token' do not include this scope).\n` +
        `Try again, or obtain a full-scope token via '${CLI_BINARY_NAME} auth login'.`,
    }
  }
  ...
  return {
    ...
    message:
      ...
      `Please log in with the correct organization: ${CLI_BINARY_NAME} auth login`,
  }
}

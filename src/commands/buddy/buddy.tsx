...
function BuddySummary({ lines, sprite, rarity }: BuddySummaryProps): React.ReactNode {
  const rarityColor = RARITY_COLORS[rarity as keyof typeof RARITY_COLORS] || 'white'

  return (
    <Box flexDirection="column">
      {sprite.map((line, i) => (
        <Text key={`sprite-${i}`} color={rarityColor}>{line}</Text>
      ))}
      <Text> </Text>
      {lines.map((line, i) => (

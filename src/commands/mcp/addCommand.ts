...
export function registerMcpAddCommand(mcp: Command): void {
  mcp
    ...
    .addOption(
      new Option(
        '--xaa',
        `Enable XAA (SEP-990) for this server. Requires '${CLI_BINARY_NAME} mcp xaa setup' first. Also requires --client-id and --client-secret (for the MCP server's AS).`,
      ...
      if (!name) {
        cliError(
          'Error: Server name is required.\n' +
            `Usage: ${CLI_BINARY_NAME} mcp add <name> <command> [args...]`,
        )
      ...
      try {
        ...
        if (xaa) {
          ...
          if (!getXaaIdpSettings()) {
            missing.push(
              `'${CLI_BINARY_NAME} mcp xaa setup' (settings.xaaIdp not configured)`,
            )
          }
          if (missing.length) {
